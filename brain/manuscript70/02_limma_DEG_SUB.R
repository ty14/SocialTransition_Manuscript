...
dlNorm_list <- list(a_countdata , p_countdata)
...
for (i in 1:length(regions)){
  ...
  saveRDS(v.dl, glue("manuscript/brain/manuscript70/results/limma_vdl_{my_region}_SUB"))
  ...
  saveRDS(q.dl,glue("manuscript/brain/manuscript70/results/results_RNAseqRDS/limma_eFDR_{my_region}_SUB_cutoff5_R{R}_contrast.RDS"))
  
  q.dl <- readRDS(glue("manuscript/brain/manuscript70/results/results_RNAseqRDS/limma_eFDR_{my_region}_SUB_cutoff5_R{R}_contrast.RDS"))
  
  
  # png(filename = glue("manuscript/brain/manuscript70/results/img/eFDR_hist_{my_region}_SUB_R{R}_contrast.png"),
  ...
  topTable(tmp1, sort.by = "P", n = Inf) %>% 
    ...
    left_join(grcm38) %>%
    filter(!is.na(symbol)) %>% 
    select(symbol,logFC,P.Value,adj.P.Val) -> limma_list$csub
  
  topTable(tmp2, sort.by = "P", n = Inf) %>% 
    rownames_to_column('ensgene') %>% 
    left_join(grcm38) %>%
    filter(!is.na(symbol)) %>% 
    select(symbol,logFC,P.Value,adj.P.Val)  -> limma_list$casc
  ...
  topTable(tmp3, sort.by = "P", n = Inf) %>% 
    ...
    left_join(grcm38) %>%
    filter(!is.na(symbol)) %>% 
    select(symbol,logFC,P.Value,adj.P.Val) -> limma_list$subasc
  
  
  saveRDS(limma_list,glue("manuscript/brain/manuscript70/results/results_RNAseqRDS/limma_{my_region}_SUB.RDS"))
  
}


limma_list<- readRDS("manuscript/brain/manuscript70/results/results_RNAseqRDS/limma_mPFC_SUB.RDS") %>% 
  map(~distinct(.)) %>% 
